...
namespace WebApi.Services.ImportExcel
{
    ...
    {
        public async static Task<string> GetAssetImportTemplate(HttpContext httpContext)
        {
            return await GetFileName(httpContext, "asset_import_template");
        }

        public async static Task<string> GetOrganizationImportTemplate(HttpContext httpContext)
        {
            return await GetFileName(httpContext, "organization_import_template");
        }

        public async static Task<string> GetUserImportTemplate(HttpContext httpContext)
        {
            return await GetFileName(httpContext, "user_import_template");
        }

        public async static Task<string> GetBookingListTemplate(HttpContext httpContext)
        {
            return await GetFileName(httpContext, "");
        }

        public static string DownloadFile(string fileName, IDataProtectionProvider provider, IConfiguration configuration)
        {
            return GetDataProtectLimitedTime(fileName, provider, configuration);
        }

        private static string GetDataProtectLimitedTime(string templateFileName, IDataProtectionProvider provider, IConfiguration configuration)
        {
