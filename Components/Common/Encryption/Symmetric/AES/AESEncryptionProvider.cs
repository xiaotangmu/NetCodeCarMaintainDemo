...
namespace Common.Encryption
{
    ...
    public sealed class AESEncryptionProvider:SymmetricEncryptionBase
    {
        ...
        /// <returns></returns>
        public static byte[] Encrypt(byte[] value, string key, string iv = null, string salt = null,
        ...
        /// <param name="encoding">编码类型，默认为<see cref="Encoding.UTF8"/></param>
        /// <returns></returns>
        public static byte[] Encrypt(byte[] value, AESKey key, Encoding encoding = null)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
            ...
            return Encrypt(value, key.Key, key.IV,encoding: encoding, keySize: key.Size);
        }
        ...
        /// <param name="keySize">密钥长度类型，默认为<see cref="AESKeySizeType.L256"/></param>
        /// <returns></returns>
        public static byte[] Decrypt(byte[] value, string key, string iv = null, string salt = null,
            Encoding encoding = null, AESKeySizeType keySize = AESKeySizeType.L256)
        {
            if (value==null)
            {
            ...
            var result = DecryptCore<AesCryptoServiceProvider>(value,
                ComputeRealValueFunc()(key)(salt)(encoding)((int)keySize),
                ComputeRealValueFunc()(iv)(salt)(encoding)(128));
            return result;
        }
        ...
        /// <param name="encoding">编码类型，默认为<see cref="Encoding.UTF8"/></param>
        /// <returns></returns>
        public static byte[] Decrypt(byte[] value, AESKey key, Encoding encoding = null)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
            ...
            return Decrypt(value, key.Key, key.IV, encoding: encoding, keySize: key.Size);
        }
        #endregion
